...
The first command argument selects the commit message language.

- `es`: write the full commit message in Spanish.
- `en`: write the full commit message in English.

If `$1` is missing or is not `es` or `en`, stop and ask the user which language to use before making any git changes.
