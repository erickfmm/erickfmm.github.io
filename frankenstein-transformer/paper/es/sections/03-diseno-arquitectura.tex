...
El campo \texttt{model\_class} determina la variante arquitectónica instanciada por el pipeline de entrenamiento. Se soportan dos opciones:
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{frankensteindecoder}: Decoder causal autorregresivo para generación de siguiente token estilo LLM. Permite el enmascaramiento de atención causal para tareas de generación secuencial de texto. Cuando se selecciona esta clase, en tiempo de ejecución se fuerza \texttt{mode='decoder'}.
\end{itemize}
...
\textbf{Selección de Patrones de Mezcladores de Secuencia}: El campo \texttt{layer\_pattern} acepta una lista ordenada de identificadores de mezcladores que abarcan ocho familias: líneas base densas (\texttt{standard\_attn}, \texttt{sigmoid\_attn}, \texttt{gqa}), recurrentes/retentivas (\texttt{retnet}, \texttt{retnet\_attn}, \texttt{mamba}, \texttt{ode}, \texttt{titan\_attn}), atención dispersa (\texttt{sparse\_transformer\_attn}, \texttt{longformer\_attn}, \texttt{bigbird\_attn}, \texttt{sparsek\_attn}, \texttt{nsa\_attn}, \texttt{sparge\_attn}, \texttt{fasa\_attn}, \texttt{msa\_attn}, \texttt{sparda\_attn}), mecanismos con compuerta (\texttt{gla\_attn}, \texttt{deltanet\_attn}, \texttt{gated\_deltanet\_attn}, \texttt{gated\_deltanet2\_attn}, \texttt{hgrn2\_attn}, \texttt{fox\_attn}, \texttt{gated\_softmax\_attn}, \texttt{kda\_attn}), compresión latente (\texttt{mla\_attn}, \texttt{gqla\_attn}, \texttt{mlra\_attn}, \texttt{tucker\_attn}, \texttt{iha\_attn}, \texttt{gta\_attn}, \texttt{mtla\_attn}, \texttt{cca\_attn}, \texttt{ccgqa\_attn}, \texttt{gma\_attn}) memoria condicional (\texttt{engram\_attn}) atención de peso rápido (\texttt{falcon1\_attn}, \texttt{falcon2\_attn}, \texttt{falcon3\_attn}, \texttt{falcon1a\_attn}, \texttt{falcon2a\_attn}, \texttt{falcon3a\_attn}) y campo geométrico (\texttt{ssog\_attn}). La taxonomía completa con referencias se proporciona en la Sección~\ref{sec:architecture-taxonomy} y el Apéndice~\ref{sec:annex-summary-tables}.
...
\node[box, fill=blue!8, minimum width=2.1cm] (cli) at (3.9,-2.95) {\textbf{CLI}\\\texttt{train / deploy / infer / web-server}};
% Main blocks
...
\node[box, fill=cyan!8, minimum width=2.0cm] (export) at (-2.6,-7.75) {\textbf{Export}\\deploy --format transformers / gguf};
% Arrows
...
\paragraph{Modelado de Lenguaje Causal (\texttt{causal\_lm}).}
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Clase de Modelo}: Establecer \texttt{task='causal\_lm'} y \texttt{model\_class='frankensteindecoder'}; el esquema y el runtime requieren la clase decoder para esta tarea (el enmascaramiento causal solo lo proporciona el decoder).
    \item \textbf{Configuración}: Reutiliza \texttt{training.optimizer}; el dataset de streaming almacena las secuencias sin enmascarar (\texttt{labels == input\_ids}) para que el trainer pueda calcular la pérdida de siguiente token desplazada.
\end{itemize}

Este soporte de tareas permite la experimentación unificada tanto en preentrenamiento tipo encoder (MLM para comprensión bidireccional), generación tipo decoder (\texttt{causal\_lm} para predicción autorregresiva del siguiente token) como en embedding de oraciones (SBERT), dentro del mismo código base.
...
La conexión residual no es solo una elección de modelado, sino también una decisión de macro-diseño. El sistema expone cuatro estrategias de conexión residual mediante el sub-objeto \texttt{model.residuals} (clave plana \texttt{residual\_type}, por defecto \texttt{standard}):

\begin{itemize}[leftmargin=*]
    \item \textbf{Residual estándar} (\texttt{residual\_type="standard"}): el residual de identidad clásico \citep{he_deep_2016}, $x_{l+1} = x_l + F(x_l, W_l)$. Sin estado, sin parámetros extra.
    \item \textbf{Sin residual} (\texttt{residual\_type="none"}, experimental): $x_{l+1} = F(x_l, W_l)$, eliminando la conexión skip por completo. Útil solo como sonda de ablación.
    \item \textbf{Residuales de Atención} (\texttt{residual\_type="full\_attn"} o \texttt{"block\_attn"}) \citep{sun_attnres_2026}: reemplazan la suma residual de coeficiente unitario fijo por atención softmax aprendida sobre la profundidad. Cada capa atiende sobre todas las salidas de capas anteriores (Full AttnRes) o sobre un pequeño conjunto de representaciones acumuladas por bloque (Block AttnRes). Añade $L \cdot C$ parámetros (un vector de query aprendido $\mathbf{w}_l$ por capa). Los queries inicializados a cero hacen que la atención inicial sea un promedio equiponderado, igualando el residual estándar en el paso cero.
    \item \textbf{Hyper-Connexiones Restringidas por Variedad} (\texttt{residual\_type} combinado con \texttt{mhc.enabled=true}, documentado por separado en el Apéndice~\ref{sec:annex-mhc}): el residual de n streams \citep{xie_mhc_2025} cuya matriz de mezcla de flujo se restringe al politopo de Birkhoff mediante Sinkhorn--Knopp.
\end{itemize}

AttnRes se integra ortogonalmente con mHC y con Mixture-of-Depths: el campo \texttt{model.residuals.mhc\_stream\_mode} selecciona si la atención sobre la profundidad corre por stream (\texttt{independent}) o conjuntamente sobre la proyección aplanada $nC$ (\texttt{joint}) cuando mHC está activo. La formulación completa, las cuatro estrategias, el cableado de implementación y los resultados reportados se detallan en el Apéndice~\ref{sec:annex-attnres}. mHC por sí solo se detalla en el Apéndice~\ref{sec:annex-mhc}.
