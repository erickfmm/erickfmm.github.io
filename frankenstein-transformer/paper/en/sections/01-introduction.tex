...
This work addresses these challenges through a unified, configuration-driven experimentation toolkit available at \url{https://github.com/erickfmm/frankenstein-transformer} that provides:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Schema-First Design}: A strict, validated configuration contract that enforces reproducibility while supporting forty-three sequence mixer variants across eight families and twenty-three optimizer families.
    \item \textbf{Architecture Agnostic Training}: Common training infrastructure supporting dense attention baselines (standard, sigmoid), recurrent alternatives (RetNet, Mamba, ODE-style blocks), adaptive depth routing (Mixture-of-Depths) \citep{zhu2026mixtureofdepthsattention}, conditional memory blocks (Engram) \citep{cheng2026conditionalmemoryscalablelookup}, manifold-constrained residual connections (mHC) \citep{xie_mhc_2025}, sparse attention patterns (Sparse Transformer, Longformer, BigBird, SparseK, NSA, SpargeAttn, FASA), gated mechanisms (GLA, DeltaNet, Gated DeltaNet, Gated DeltaNet-2, HGRN2, FoX, Gated Softmax), and fast-weight attention (Falcon-1/2/3 and Falcon-1A/2A/3A) \citep{zhang_falcon_2026}.
...
\begin{center}
\texttt{frankenstein-transformer}
\end{center}
with subcommands for encoder and decoder workflows:
\texttt{train} (including SBERT-style sentence-embedding objectives via \texttt{training.task: sbert}), \texttt{deploy} (quantized/standard artifacts plus HuggingFace Transformers and BitNet GGUF export formats), \texttt{infer}, and \texttt{web-server}.
